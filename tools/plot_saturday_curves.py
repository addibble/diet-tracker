"""Plot strength curves for Saturday's workout with progressive refit.

For each exercise done on Saturday (session 388), shows:
...
DB_PATH = Path(__file__).resolve().parent.parent / "production_backup_2026-04-11_223242.db"
SESSION_ID = 388
SESSION_DATE = date(2026, 4, 11)


def load_session_exercises(conn):
    """Load exercises and their sets from Saturday's session."""
    c = conn.cursor()
    rows = c.execute("""
...
def load_historical_sets(conn, exercise_id, days=30):
    ...
    c = conn.cursor()
    cutoff = date(2026, 4, 11 - days) if days <= 11 else date(2026, 3, 12)
    # More robust cutoff
    ...
    # Separate today's sets from historical
    historical = []
...
def fit_from_observations(weights, reps_fail, confidences, ages, label=""):
...
def plot_exercise(ax, ex_data, conn):
    ...
    hist_w = [h["weight"] for h in hist]
    hist_r = [h["reps"] + (10.0 - h["rpe"]) for h in hist]
    hist_c = [_rpe_confidence(h["rpe"]) for h in hist]
    hist_a = [float(h["age_days"]) for h in hist]

    # Apply t-test filter to historical
    hist_w, hist_r, hist_c, hist_a = _filter_stale_sessions(
        hist_w, hist_r, hist_c, hist_a
    )

    # Determine weight range for plotting
    all_weights = hist_w + [s["weight"] for s in sets]
    if not all_weights:
    ...
    labels = ["Historical", "After Set 1", "After Set 2", "After Set 3"]
    fits = []

    # Historical fit (no session sets)
    hist_fit = fit_from_observations(hist_w, hist_r, hist_c, hist_a, "hist")
    fits.append(("Historical", hist_fit, colors[0]))

    # Progressive refits after each set
    ...
    for i, s in enumerate(sets):
        ...
        fits.append((f"After Set {i+1}", refit, colors[min(i+1, 3)]))
    ...
    for i, s in enumerate(sets):
        ...
        ax.scatter(s["weight"], r_fail, s=60, zorder=5,
                   color=colors[min(i+1, 3)], edgecolors="white", linewidth=0.8)
        ax.annotate(f"S{i+1}: {s['weight']}×{s['reps']}\nRPE {rpe:.0f}",
                    (s["weight"], r_fail),
                    textcoords="offset points", xytext=(6, 4),
                    fontsize=5.5, color=colors[min(i+1, 3)])
...
def main():
    ...
    fig.suptitle(
        "Saturday 4/11 Workout — Strength Curves with Progressive Refit\n"
    ...
    out = Path(__file__).parent / "saturday_curves.png"
